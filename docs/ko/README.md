...
웹페이지, 이미지, 앱 화면의 글을 선택하거나 복사하기 어려울 때가 있어요. SwiftyCrow는 화면에서 글자를 읽어, 사용하던 앱을 그대로 보면서 번역할 수 있게 해요.

- **화면에서 바로 번역:** 파일로 저장하거나 글을 다른 앱에 옮기지 않고 필요한 영역을 선택해요.
- **내용에 맞는 번역 방식:** 한 번 읽고 가져올 때는 캡처 번역을, 바뀌는 글을 이어서 읽을 때는 실시간 번역을 사용해요.
...
처음 실행하면 빠른 시작에서 화면 기록 권한, 언어 다운로드, 첫 캡처를 안내해요. 시스템 설정을 열기 전에 단계와 선택한 언어를 저장하므로, macOS가 앱을 종료하고 다시 열어도 이어서 진행할 수 있어요. 설정 → 일반 → 권한에서도 현재 상태를 확인할 수 있어요.

<a id="usage"></a>
## 사용 방법

1. 설정(`⌘,`)에서 **원문 언어**와 **번역 언어**를 선택해 주세요.
2. **영역 캡처:** 제어창이나 단축키로 **캡처 번역**을 시작한 뒤 글 위를 드래그해 주세요. **Space**를 누르면 창 전체를 선택할 수 있어요. 결과 창은 제목 부분을 잡아 옮길 수 있어요. `⌘+` / `⌘−`로 확대·축소하고, 배율을 누르거나 `⌘0`로 창에 맞출 수 있어요. 100%는 이미지 픽셀 하나가 화면 픽셀 하나에 대응하는 크기예요. `⌘S`는 저장, `⌘C`는 이미지 복사, `⌘O`는 원문 복사, `⌘T`는 번역문 복사, `Esc`는 닫기예요. 저장·복사는 현재 확대율이나 이동 위치와 관계없이 전체 이미지를 원본 해상도로 담아요.
3. **실시간 번역:** **실시간 번역** 버튼을 눌러 영역이나 창을 고르세요. 다시 누르면 새 영역을 선택할 수 있어요. 옆의 **오버레이 표시** 스위치는 선택한 영역을 켜고 끄며, 아직 영역을 고르지 않았다면 비활성화돼요. 오버레이의 **실시간** 버튼으로 멈추거나 다시 시작하고, `⌘C`로 번역문 전체를 복사하고, **×**로 닫을 수 있어요.
4. **번역 표시·숨기기:** **오버레이 표시** 스위치로 선택한 영역의 번역을 켜고 끌 수 있어요. 끄면 캡처와 번역이 멈추고, 다시 켜면 같은 영역에서 이어서 번역해요.

새로 설치하면 캡처는 ⇧⌘1, 실시간 번역 영역 선택은 ⇧⌘2로 시작할 수 있어요. 빠른 시작에서 설정하거나, 설정 → 단축키에서 캡처·오버레이·저장·복사 단축키를 모두 바꿀 수 있어요.

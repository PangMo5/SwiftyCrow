...
수정 키는 `cmd`, `ctrl`, `alt`(option), `shift`예요. 키에는 영문자, 숫자, `tab`, `return`, `space`, 방향 키(`left`/`right`/`up`/`down`), 문장 부호 등을 쓸 수 있어요. 항목을 생략하면 해당 동작에 단축키를 지정하지 않아요.
...
메뉴 막대의 **실시간 번역…** 또는 `liveOverlay` 단축키로 영역이나 창을 선택하면 바로 실시간 번역이 시작돼요. 본문 영역의 클릭은 뒤쪽 앱으로 전달되고, 제어 버튼·크기 조절 가장자리·열린 정보창은 입력을 받아요. **실시간**으로 멈추거나 다시 시작하고 **×**로 닫을 수 있어요.

한 번 배치한 영역은 기억해요. `toggleLiveOverlay` 단축키나 메뉴의 **지난 영역에 표시** / **실시간 번역 숨기기**로 다시 선택하지 않고 같은 위치에서 켜고 끌 수 있어요. 숨기면 캡처와 번역을 모두 멈추고, 다시 표시하면 저장한 영역에서 실시간 번역을 시작해요. 영역을 미리 정해 두고 필요할 때 번역하는 방식이에요.
...
모든 값은 앞서 설명한 skhd 형식의 단축키 문자열이에요. 전역 항목을 생략하면 단축키를 지정하지 않아요. 이것이 기본 동작이에요.

...
修饰键为 `cmd`、`ctrl`、`alt`（option）和 `shift`。按键可使用字母、数字、`tab`、`return`、`space`、方向键（`left`/`right`/`up`/`down`）、标点等。省略某项可使该操作不绑定快捷键。
...
从菜单栏的**实时翻译…**或 `liveOverlay` 快捷键选择区域或窗口，即可开始实时翻译。正文区域的点击会传递给下方应用，控件、调整大小的边缘和已打开的信息窗口会接收输入。用**实时**按钮暂停或继续，**×**关闭。

放置后的区域会被记住。使用 `toggleLiveOverlay` 快捷键，或菜单中的**在上次区域显示**／**隐藏实时翻译**，即可在同一位置切换，无需重新选择。隐藏会停止所有截图和翻译，再次显示会恢复该区域并开始实时翻译，实现预设区域、按需使用。
...
所有值均采用上文的 skhd 风格快捷键字符串。省略全局键表示不绑定该操作，这是默认行为。

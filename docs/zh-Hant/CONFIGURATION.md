...
修飾鍵為 `cmd`、`ctrl`、`alt`（option）與 `shift`。按鍵可使用字母、數字、`tab`、`return`、`space`、方向鍵（`left`/`right`/`up`/`down`）、標點等。使用空字串即可解除該操作的快捷鍵。
...
點選選單列中的**即時翻譯**按鈕，或使用 `liveOverlay` 快速鍵選取區域或視窗。再次點選按鈕可選取新區域。內容區域的點選會傳遞給下方 App；控制按鈕、調整大小的邊緣和開啟的資訊面板會接收輸入。使用 **即時** 暫停或繼續，使用 **×** 關閉。

`toggleLiveOverlay` 快速鍵和**顯示浮動翻譯**開關可顯示或隱藏所選區域的翻譯。關閉時停止擷取和翻譯，重新開啟時使用記住的區域。選取區域前，開關處於停用狀態；沒有儲存的區域時，快速鍵也不會執行操作。
...
所有值皆為上述 skhd 格式的快捷鍵字串。擷取預設為 ⇧⌘1，即時區域選取預設為 ⇧⌘2；設定中省略這些鍵時也會採用預設值。明確指定的自訂快捷鍵會保留。空字串可清除任何快捷鍵，包括其預設值。

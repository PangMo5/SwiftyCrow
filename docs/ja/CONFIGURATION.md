...
修飾キーは`cmd`、`ctrl`、`alt`（option）、`shift`です。キーには英字、数字、`tab`、`return`、`space`、矢印キー（`left`/`right`/`up`/`down`）、記号などを使えます。項目を省略すると、その操作にはショートカットを割り当てません。
...
メニューバーの**ライブ翻訳…**または`liveOverlay`ショートカットで範囲やウインドウを選ぶと、ライブ翻訳が始まります。本文範囲のクリックは背後のアプリに届き、操作ボタン・サイズ変更の端・開いている情報ポップオーバーは入力を受け取ります。**ライブ**で一時停止・再開し、**×**で閉じられます。

一度配置した範囲は記憶されます。`toggleLiveOverlay`ショートカットやメニューの**前回の範囲に表示**／**ライブ翻訳を非表示**で、選び直さず同じ場所の表示を切り替えられます。非表示にするとキャプチャと翻訳をすべて停止し、再表示すると保存した範囲でライブ翻訳を始めます。範囲を決めて、必要なときだけ翻訳する使い方です。
...
値はすべて前述のskhd形式のショートカット文字列です。グローバルな項目を省略すると、ショートカットを割り当てません。これが初期状態です。
